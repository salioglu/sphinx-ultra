...
**Audit-verified status as of 2026-08-07** (v0.3.0 + the 2026-08 M1 waves 1–4).
...
| RST parsing | 🟡 | The binary still runs the line-scanner (`src/parser.rs`) — same gaps as before (no inline markup/lists/tables/footnotes/targets in the build path; `::` drops the introducing paragraph). **M2 wave 1 (2026-08) landed the replacement as 🧩 built-not-wired, differentially verified**: `src/doctree/` (generic-node IR with spans, byte-parity `pformat`, docutils-exact `make_id`/`fully_normalize_name`/id registry) + `src/rst/` (recursive-descent block parser: sections with full error recovery, transitions, bullet/enumerated lists incl. docutils sequence resolution, definition lists + classifiers, block quotes + attribution, literal `::`/quoted/doctest/line blocks, comments, all four hyperlink-target forms, system_messages byte-matched). Zero divergence on a committed 175-case fixture vs docutils 0.22.4 parse layer (`tests/doctree_differential.rs`; regen: `uv run --python 3.12 --with docutils==0.22.4 python tools/gen_doctree_fixture.py`); proptest totality suite (`tests/rst_proptest.rs`), MAX_NEST_DEPTH=200 guard. **Wave 2 (2026-08) landed the full inline parser** (docutils Inliner port: emphasis/strong/literal with escape `\x00` semantics and quoted-start suppression, named/phrase/anonymous/embedded references with inline targets, built-in roles incl. PEP/RFC, footnote/citation/substitution references, standalone URIs/emails from generated docutils scheme tables) **plus footnote/citation definitions, field lists, full option lists, and grid + simple tables** (docutils tableparser ports incl. the malformed-table message families and cell-content nested parsing). Differential fixture now 426 cases, still zero divergence; punctuation/urischeme tables generated from docutils via `tools/gen_punctuation_tables.py`. Wave 3: directive machinery + substitution definitions + wiring into `Parser::parse`. |
