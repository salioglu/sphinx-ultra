...
**518 tests, all passing** (as of M2 wave 4). Every generator below is pinned to
...
| Suite | Status |
|---|---|
| Unit tests (lib + bin) | ✅ 422 passing (415 lib + 7 bin) |
...
| Sphinx doctree differential suite (real read phase) | ✅ 314 generated cases vs a `sphinx-build` 9.1.0 read phase, zero divergence; regenerate with `uv run --python 3.12 --with 'sphinx==9.1.0' --with 'docutils==0.22.4' python tools/gen_sphinx_fixture.py` |
| Environment differential suite | ✅ 22 tests over 15 projects / 47 documents vs a real `SphinxTestApp` build, zero divergence on every compared key, cold and warm (exemption tables above); same `uv` invocation with `tools/gen_env_fixture.py` |
...
| Doctree serde / interner-cap / math-name suites | ✅ 4 passing — bincode round-trip, the interner's bound, and sphinx `math` name variants |
...
| `tests/e2e_cli.rs` | ✅ 45 passing — the real binary against fixture projects: exit codes, warning text, output trees, `--config` routing, sphinx-build mode, incremental/dependency rebuilds |
