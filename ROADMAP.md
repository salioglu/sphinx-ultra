...
| RST parser | **Working (docutils-fidelity, wired)** | M2 wave 3 (2026-08-13): the binary runs the docutils-fidelity parser (`src/rst/`, sphinx mode); the M1 line-scanner is deleted. Full block+inline grammar (waves 1–2), docutils-exact directive machinery with typed option converters, the docutils built-in directive set, and substitution definitions — zero divergence across a committed 653-case parse-layer differential fixture vs docutils 0.22.4 (`tools/gen_doctree_fixture.py`). Sphinx set (toctree, versionmodified family, seealso, code-block/highlight, only, rst-class, math, index, hlist, glossary, xref + pep/rfc/cve/cwe roles) verified against a second oracle: 277 cases at zero divergence vs a real sphinx-build 9.1.0 read phase (`tools/gen_sphinx_fixture.py`). Wave-3 deferrals recorded in the wave notes: literalinclude/include, object descriptions, ifconfig, meta, rst_prolog/epilog/default_role. |
