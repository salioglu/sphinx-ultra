...
| Validation systems | **Two of three live** | Directive/role validation runs in every build (default on, false-positive heuristics fixed/demoted, Unknown silent). Cross-reference validation is no longer the M1 heuristic scanner: wave 4 replaced it with the std domain + Sphinx's own resolution pass, and with it Sphinx's `warn_dangling` semantics — **`:doc:` and `:ref:` now warn about broken targets without `-n`**, because Sphinx warns on those two reftypes unconditionally and reserves `-n`/`nitpicky` for the rest. That is a behavior change for existing projects (see CHANGELOG). The M1 `src/domains/` module was deleted once nothing called it. Constraint engine stays library-only until sphinx-needs items exist (M4) — no `ContentItem` producer yet; its always-success placeholder trait impls were deleted so future wiring can't silently no-op. |
