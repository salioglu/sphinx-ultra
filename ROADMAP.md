...
The single most important fact is unchanged since 0.3.0: **the shipped build path
emits the raw RST/Markdown source, HTML-escaped, inside `<html><body>` — it does not
yet render documentation.** Everything *behind* that placeholder is now real: M2
waves 1–3 replaced the line-scanner with a docutils-fidelity parser, and wave 4
(2026-08-31) made the read → merge → resolve → write pipeline real — a serialized
`BuildEnvironment`, the toctree graph, numbering, the std domain, genindex data,
`objects.inv`, and intersphinx, each pinned against a real `sphinx-build` 9.1.0
oracle. What remains **dead code from the binary's execution path** — real modules
with real tests that `sphinx-ultra build` never calls — is the write-side stack the
M2 wave-5 HTML writer revives: the Sphinx-mirroring `HTMLBuilder`, the minijinja
`TemplateEngine`, `SearchIndex`, and the `objects.inv` *writer* (its reader is live,
feeding intersphinx).
...
| Incremental cache | **Working (dependency-driven)** | M1: hits write output, `--clean --incremental` safe, size/expiry knobs plumbed, config-change invalidation, warm-rebuild deadlock fixed, eviction honestly named. M2 wave 4 replaced mtime-only staleness with Sphinx's env-level outdated computation (`BuildEnvironment::get_outdated_files`): added/changed/removed documents plus per-document file dependencies (`src/env/dependencies.rs`, Sphinx's `relfn2path` + asset-collector `note_dependency`; images are the whole population until `include`/`literalinclude` land in wave 4.5). Persisted doctrees carry a magic+version header, so a blob written by an older format is an honest cache miss instead of a silently mis-decoded doctree (bincode has no self-description). Warm-equals-cold is asserted, not assumed — but narrowly: six tests in `tests/env_differential.rs` rebuild their own two- and three-document projects over their own cache and require the warm build's std-domain registries, toctree graph, resolved doctrees and warnings to equal the cold build's. The 15-project oracle corpus is not in that loop; it is built once, cold. |
| RST parser | **Working (docutils-fidelity, wired)** | M2 wave 3 (2026-08-13): the binary runs the docutils-fidelity parser (`src/rst/`, sphinx mode); the M1 line-scanner is deleted. Full block+inline grammar (waves 1–2), docutils-exact directive machinery with typed option converters, the docutils built-in directive set, and substitution definitions — zero divergence across a committed 663-case parse-layer differential fixture vs docutils 0.22.4 (`tools/gen_doctree_fixture.py`). Sphinx set (toctree, versionmodified family, seealso, code-block/highlight, only, rst-class, math, index, hlist, glossary, xref + pep/rfc/cve/cwe roles) verified against a second oracle vs a real sphinx-build 9.1.0 read phase (`tools/gen_sphinx_fixture.py`). Wave 4 (2026-08-31) added the std-domain directives — `program`/`option` (incl. `[=value]` and multi-name forms), `envvar`, `confval` with `:type:`/`:default:`, `describe`/`object`, `default-domain` — on a generic **object-description anatomy** (`desc`/`desc_signature`/`desc_name`/`desc_addname`/`desc_annotation`/`desc_content`, the `:no-index:` option family, `PropagateDescDomain`), plus glossary-term ids through Sphinx's `make_id` and index entries following `process_index_entry`. That fixture now stands at **316 cases, zero divergence**. Still deferred (wave 4.5): `literalinclude`/`include`, py-domain object descriptions. Also deferred: ifconfig, meta, rst_prolog/epilog/default_role. |
| BuildEnvironment & resolution | **Working** | M2 wave 4 (2026-08-31). The build is a real read → merge → resolve → write pipeline over a serialized `BuildEnvironment` (bincode, `ENV_VERSION`-stamped, per-document doctrees under the `-d` dir). Live: the global toctree graph with relations (parents/prev/next) and Sphinx's consistency warnings (nonexisting/excluded entries, self-reference, circular toctrees, orphans, "document isn't included in any toctree"); section + figure numbering (`numfig`, `numfig_secnum_depth`, `numfig_format`, `:numbered:`); the **std domain** — label/term/option/envvar/confval/program collection and `ref`/`numref`/`doc`/`term`/`option`/`envvar` resolution with Sphinx's duplicate-label and dangling-reference warnings; `nitpick_ignore`/`nitpick_ignore_regex`. Evidence: an environment-layer differential oracle (`tools/gen_env_fixture.py`, `tests/env_differential.rs`) that builds **15 multi-document projects** (47 documents) with a real `sphinx-build` 9.1.0 and compares every recorded key — `tocs`, `toc_num_entries`, `toctree_includes`, `files_to_rebuild`, relations, `toc_secnumbers`/`toc_fignumbers`, the std registries, index entries, genindex, the whole warning stream, and each document's fully cross-reference-resolved doctree — at **zero divergence** over one cold build per project (warm-equals-cold is the separate, narrower claim above). Two strict, self-cleaning exemption tables remain, each pinned to the work that closes it: 11 of 15 projects' warning streams match byte-for-byte (3 differ only by `image file not readable`, 1 by the write-phase circular-toctree warning), and 20 of 47 documents' resolved doctrees match byte-for-byte (the other 27 carry an unresolved `toctree` node, an image without `candidates`, or an unapplied `PropagateTargets`). An exemption whose project *stops* diverging fails the test, so the table shrinks on its own. py-domain object registration is wave 4.5 (see §4 M2). |
| genindex & domain indices | **Working (data)** | M2 wave 4: `index` directive/role entries are collected per Sphinx's `IndexDomain.process_doc` (incl. the `invalid … index entry` warning) and assembled by a port of `IndexEntries.create_index` — single/pair/triple/see/seealso, `!main` promotion, Symbols/`_` grouping, sub-entry ordering, and the "dropped index entry" notice. Pinned by the env oracle's `genindex` key across the corpus and by `src/env/genindex.rs`'s own tests. **No `genindex.html` is written** — the page render needs the wave-5 HTML writer. Domain indices (py-modindex) await the py domain in wave 4.5. |
...
| objects.inv | **Working (reader + writer)** | M2 wave 4 rewrote both halves bytewise. The reader is binary-safe end to end (the M1 reader lossily UTF-8-converted the zlib payload and then split it on `str::lines`, corrupting any inventory whose compressed bytes contained a stray `\r`/`\n` or non-UTF-8 sequence — a property real inventories have); it handles v1 and v2, `$`-suffix anchor expansion, `-` dispname expansion, and reproduces Sphinx's own `ValueError` texts verbatim. The writer mirrors `InventoryFile.dump` (header escaping, zlib level 9, alphabetical domain order, sorted `get_objects()`, suffix compression). Evidence: `tests/fixtures/inventories/` — 12 committed `.inv` files (4 written by a real `sphinx-build` 9.1.0, 3 handcrafted-valid, 5 handcrafted-malformed) whose expectations are whatever Sphinx's own `InventoryFile.loads` returns or raises, checked by `tests/inventory_roundtrip.rs`. **The writer has no production call site**: nothing writes an `objects.inv` into a build output until the wave-5 HTML writer's finish task. The reader is live — intersphinx is its consumer. |
| intersphinx | **Working** | M2 wave 4: `intersphinx_mapping` (named and unnamed, with Sphinx's `ConfigError` texts for malformed entries), inventory loading with the on-disk cache and its refresh semantics, `intersphinx_disabled_reftypes`, the `:external:`/`:external+inv:` role family, the missing-reference hook, and the shared HTTP configuration group (`tls_verify`, `tls_cacerts`, `user_agent`, `intersphinx_timeout`). Verified **offline**: 54 tests across `src/intersphinx/` and `src/env/resolve/intersphinx_tests.rs`, driving the real code over the committed `.inv` fixtures through an injected fetcher. Remote fetching is implemented (`src/intersphinx/fetch.rs`, `ureq` + rustls) and deliberately has **no test that touches the network** — the seam is a trait so the suite stays hermetic. |
...
| Validation systems | **Two of three live** | Directive/role validation runs in every build (default on, false-positive heuristics fixed/demoted, Unknown silent). Cross-reference validation is no longer the M1 heuristic scanner: wave 4 replaced it with the std domain + Sphinx's own resolution pass, and with it Sphinx's `warn_dangling` semantics — **all seven `warn_dangling` std reftypes now warn about broken targets without `-n`**: `ref`, `numref`, `doc`, `term`, `keyword`, `option`, `confval` (`domains/std/__init__.py:748-766`), with `-n`/`nitpicky` reserved for everything else. That is a behavior change for existing projects (see CHANGELOG). The M1 `src/domains/` module was deleted once nothing called it. Constraint engine stays library-only until sphinx-needs items exist (M4) — no `ContentItem` producer yet; its always-success placeholder trait impls were deleted so future wiring can't silently no-op. |
| Build-path validation | **Working** | Toctree entry, glob, self-reference, circular, multi-parent and orphan checks, plus "document isn't included in any toctree", now come from the real toctree graph with Sphinx's exact texts, categories (`[toc.not_readable]`, `[ref.ref]`, …) and **locations** — a toctree warning is reported at the `.. toctree::` directive line, as Sphinx reports it, not at the offending entry's line. Directive/role validation and std-domain cross-reference resolution run on every build; `-n`/nitpicky widens resolution warnings to the non-`warn_dangling` reftypes. All findings flow through the `-W`/`-w` pipeline. |
...
| CI/release | **Working, with one vacuous job** | Fixed 2026-08: publish gated on validation+build, Cargo.lock committed + `--locked` everywhere, SHA-256 checksums + install.sh verification, aarch64-linux artifact built, vacuous `integration_test.rs` replaced by a real E2E suite. **Known defect (fix open as PR #54, which is repo-wide and lands ahead of the wave-4 branch):** the `MSRV (1.85)` job and the `beta` matrix leg do not test what they name. Both set the toolchain with `rustup default`, which rustup ranks *below* the repo's `rust-toolchain.toml` (`channel = "stable"`), so both jobs compile with stable. Verified locally: in this checkout `rustup show active-toolchain` reports `stable … (overridden by rust-toolchain.toml)`, and only `RUSTUP_TOOLCHAIN=1.85` — which outranks the file — actually selects 1.85. Until #54 merges the MSRV is checked by hand (`RUSTUP_TOOLCHAIN=1.85 cargo check --locked --all-targets`, green as of wave 4) and not by CI. |
| Dependencies | **Pruned 2026-08** | 16 zero-call-site deps removed (incl. pyo3/pythonize, which carried two RUSTSEC advisories and linked libpython into every build). Wave 4 added two with real call sites: `bincode` (environment + doctree persistence) and `ureq` with rustls (intersphinx inventory fetching; `resolver = "3"` keeps MSRV-incompatible versions out). minijinja/flate2 remain — flate2 is now live in the `objects.inv` reader; minijinja still serves the built-not-wired template stack. |
...
**Sequencing amendment (recorded 2026-08-31, M2 wave 4).** Scope is unchanged; the
order inside M2 is not. Wave 4 delivered the environment, resolution, `objects.inv`
and intersphinx bullets above, but **py-domain object registration moved to a new
wave 4.5**, together with everything that depends on it:

- py-domain object registration from `py:*` directives, and the object-signature
  config family (`maximum_signature_line_length` + per-domain variants,
  `toc_object_entries`/`toc_object_entries_show_parents`,
  `add_function_parentheses`);
- domain indices — `py-modindex`;
- `include`/`literalinclude`, deferred by wave 3 (both need per-line provenance),
  which are also the remaining file-dependency sources for incremental rebuilds.

Reason: wave 3 deferred object descriptions, so entering wave 4 there was no doctree
substrate to register *from* — `.. py:function::` was an unknown-directive error, and
registration with nothing to register is untestable. Wave 4 built the generic
object-description anatomy and proved it on the std domain's directives (`option`,
`envvar`, `confval`, `describe`, `program`); wave 4.5 puts the py domain on top of it.
Consequences visible today: the nitpicky pass still counts python-domain references
and reports them once as unvalidatable, and image `uri`s are the only file
dependencies the incremental build tracks.
